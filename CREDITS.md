...
> ## 关于本仓库
>
> **本仓库是 [tonyd2wild/DeepSeek-v4-Flash-0731-DSpark-1M-NVFP4-KV-2x-DGX-Spark](https://github.com/tonyd2wild/DeepSeek-v4-Flash-0731-DSpark-1M-NVFP4-KV-2x-DGX-Spark) 的下游部署记录**，不是那份 recipe 的替代品。
>
> 本仓库自己贡献的部分：一套双 GX10 上的实测数据与调优记录、两个测量工具
> （`bench-baseline.py` / `loadtest.py`）、以及启动脚本自检部分的重写。
> 各文件的许可归属见 [NOTICE](NOTICE)。
>
> **下面这份清单来自上游仓库，记录的是上游 recipe 各部分的来源**，原样保留。

本 recipe 汇集了多项公开工作。复用 recipe、补丁或基准数据时，请注明上游作者。
