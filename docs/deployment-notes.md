...
**生产配置，5×500 token 预热后，`stream:false`，t=0，3 次取中位数：**
...
> **本节的数字来自上游竞赛，不是本机实测。** 本文其余性能数字都是本机测的，这一节是例外，特此标明。

上游数据：**预填充随深度变快** —— 8K → 1,513、32K → 2,284、100K → **2,639** tok/s；10 个配置里波动不超过 ±2%，**计算受限，调不动**。

**本机做过什么、没做过什么：**

- ❌ **没有做过专门的预填充基准**。`bench-baseline.py` 的四条提示词都只有约 28 个输入 token，预填充可忽略
- ⚠️ `loadtest.py` 的 mixed 模式确实带一条约 5000 token 的长提示词，但它报的 `输入吞吐` 是 **`总输入 token ÷ 总墙钟`**，分母包含解码时间 —— **那不是预填充吞吐，会严重低报**，只能用来横向比较不同并发下的相对变化

#### 怎么测（脚本已就位，结果待填）

**不要用 `max_tokens: 1` 隔离预填充** —— 那是合成条件。开着 chunked prefill 时预填充本来就和解码共享调度步，隔离测出来的数偏乐观，不反映真实使用。

改用两个在真实条件下成立的量，`scripts/measure-prefill.py` 两种模式各对应一个：

```bash
./scripts/measure-prefill.py ttft --md        # TTFT vs 提示词长度
./scripts/measure-prefill.py blocking --md    # 长预填充在途时短请求被堵多久
```

**① TTFT（`ttft` 模式）** —— 用户直接感受到的量，扫 1K / 8K / 32K / 100K 四档，每档 3 次取中位数。同时读引擎自己报的 `Avg prompt throughput`（vLLM 内部就把预填充和解码分开统计了，不需要合成设置）。

> 这里用**流式**测 TTFT 是正确的，和第 6.1 节"测吞吐必须非流式"不冲突 —— 两者是不同指标：吞吐要数 token，而流式 chunk 装的是整步接受的多个 token；TTFT 只关心第一个 chunk 何时到达，只有流式拿得到这个时刻。

**② 短请求被堵多久（`blocking` 模式）** —— 先测空闲时短请求的 TTFT 作基准，再在一条长请求预填充在途时重测，看比值。

**这一项才是混合负载的真问题**，也是判断第 8.4 节那两个预填充调度参数值不值的唯一依据：那节从源码推出了 `long_prefill_token_threshold` 的机制（每步 token 硬上限，8 步变 59 步），但**"短请求实际被堵多久"这一侧从未量过**。

- 比值接近 1 → chunked prefill 已经让短请求插进去了，那两个参数没有必要
- 比值很大 → 长预填充在独占调度步，此时才值得重新考虑

**结果填回本节**，并把配置四元组和当次日期一并记上（第 6.5 节）。

### 5.7 KV 池：期望值与噪声

生产配置下 KV 池约 **1.24M–1.31M token**，`Maximum concurrency` 约 2.44–2.50x。
...
它确实能让短交互不被长预填充堵死 —— 每步只吃 1024 预算，剩下 7168 留给别的请求。**但代价是"预填充计算受限、随深度变快"（8K→1,513、100K→2,639 tok/s）被直接放弃。**
